...
import {
  ...
  UploadCloud, Loader2, FileText, Check, BadgeCheck, BadgeX
} from 'lucide-react';
...
const LandlordSettings = () => {
  const { user, fetchUserProfile } = useAuth(); // Assuming your auth context has a way to refresh user data
  ...
  // --- 1. INITIALIZE DATA ---
  useEffect(() => {
    // Populate form with existing user data from context
    if (user) {
      setProfileData({
        name: user.name || '',
        phone: user.phone || '',
  ...
  // Helper to clear alerts when switching tabs
  ...
  const handleProfileUpdate = async (e) => {
    ...
    try {
      ...
      // Refresh the global auth state so the navbar updates instantly
    ...
    <div className="max-w-4xl mx-auto pb-12 animate-in fade-in duration-300">
      ...
      {activeTab === 'verification' && (
        <div className="bg-white p-6 md:p-8 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className={`p-6 rounded-xl border mb-8 flex flex-col md:flex-row items-center gap-4 ${verificationStatus === 'verified' ? 'bg-green-50 border-green-200 text-green-800' :
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-sm font-medium opacity-90">
                {verificationStatus === 'verified' && "Your identity is verified. You can list and manage properties without restrictions."}
                {verificationStatus === 'pending' && "Your documents are currently under review by our admin team. This usually takes 24-48 hours."}
                {verificationStatus === 'unverified' && "You must verify your identity using a Government ID (Aadhar or PAN) before you can publish properties."}
          ...
          {/* Upload Form (Only show if not verified/pending) */}
                ...
                <div className="pointer-events-none flex flex-col items-center">
                  {verificationFile ? (
                    ...
                    <>
                      ...
                      <p className="font-bold text-blue-900">Click to upload your Aadhar, PAN Card and more.</p>
                      <p className="text-sm text-blue-600 mt-1">Supports PDF, JPG, PNG (Max 10MB)</p>
                    </>
                  )}
                </div>
              </div>
